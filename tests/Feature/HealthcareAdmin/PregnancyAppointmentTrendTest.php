...
test('pregnancy admin can see appointment trends chart', function () {
    // Ensure roles exist
    Artisan::call('db:seed', ['--class' => RoleSeeder::class]);

    $adminRoleId = \App\Models\Role::where('name', 'healthcare_admin')->first()->id;

    $pregnancyAdmin = User::factory()->create([
        'role_id' => $adminRoleId,
        'admin_category' => AdminCategoryEnum::Pregnancy,
        'status' => 'active',
    ]);

    // Create pregnancy care service
    $service = Service::factory()->create([
        'category' => 'pregnancy_care',
        'name' => 'Pregnancy Care Service',
    ]);
    ...
    actingAs($pregnancyAdmin)
        ->get(route('healthcare_admin.dashboard'))
        ->assertOk()
        ->assertSee('Pregnancy Care Appointments', false)
...
test('non-pregnancy admin does not see pregnancy appointment trends chart', function () {
    // Ensure roles exist
    Artisan::call('db:seed', ['--class' => RoleSeeder::class]);
...
test('pregnancy appointment trends include predictions for next two months', function () {
    // Ensure roles exist
    Artisan::call('db:seed', ['--class' => RoleSeeder::class]);
    ...
    $response = actingAs($pregnancyAdmin)->get(route('healthcare_admin.dashboard'));
...
test('pregnancy appointment trends display correct data format', function () {
    // Ensure roles exist
    Artisan::call('db:seed', ['--class' => RoleSeeder::class]);
